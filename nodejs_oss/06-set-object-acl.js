  ...
  if (err) {
    ...
    s3.putObject(newObject, function (err, data) {
      if (err) {
        ...
        // 设置对象的ACL
        s3.putObjectAcl(aclRequest, function (err, data) {
          if (err) {
            console.log("Error of Put Object ACL: " + err);
          } else {
            console.log("Set acl public-read-write for object " + objectName + " in bucket " + bucketName);
          }
        })
      }
    })
