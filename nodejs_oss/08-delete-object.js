  ...
  if (err) {
    ...
    s3.putObject(newObject, function (err, data) {
      if (err) {
        ...
        // 删除一个对象
        s3.deleteObject({Bucket: bucketName, Key: objectName}, function(err, data) {
          if (err) {
            console.log("Error of Delete Object: " + err);
          } else {
            console.log("Delete object " + objectName + " from bucket " + bucketName);
          }
        })
      }
    })
